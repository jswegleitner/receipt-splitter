...
- **Icons** — inline SVG components (Lucide paths) defined in `index.html` via the `LucideSvg` helper; no icon CDN. (Lucide-via-CDN was removed because lucide@1.x's `createIcons()` didn't bind to its named exports, so icons never rendered.)
